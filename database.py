...
def get_admin_name(admin_id):
    conn = sqlite3.connect(DB)
    cur = conn.cursor()
    cur.execute("SELECT username FROM players WHERE user_id=? AND is_admin=1", (admin_id,))
    row = cur.fetchone()
    conn.close()
...
def add_warn(user_id, reason=None, admin_id=None):
    ...
    new_by_entry = f"[{timestamp}] {admin_name} (ID:{admin_id if admin_id else 'система'})"
...
def mute_player(user_id, hours=2, reason=None, admin_id=None):
    ...
    conn.execute("""
        UPDATE players SET 
            ...
            mute_by=?
        WHERE user_id=?
    """, (until, reason, admin_name, user_id))
    conn.commit()
    conn.close()
    return until
